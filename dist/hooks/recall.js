...
const STARTUP_PROMPT_RE = /A new session was started via \/new or \/reset[\s\S]*?Do not mention internal steps, files, tools, or reasoning\./i;
const CURRENT_TIME_RE = /(?:^|\n)Current time:[^\n]*/gi;
const COMMAND_ONLY_RE = /^\s*(?:\{\/|\/)[a-z0-9_-]+\s*$/i;
function normalizeWhitespace(text) {
    return text.replace(/\r/g, "").replace(/\n{3,}/g, "\n\n").trim();
}
function extractRecallQuery(rawPrompt) {
    if (STARTUP_PROMPT_RE.test(rawPrompt))
        return null;
    ...
    for (let i = blocks.length - 1; i >= 0; i--) {
        ...
        if (COMMAND_ONLY_RE.test(block))
            return null;
        if (block.length >= 5)
            return block.slice(-1200);
    }
    const fallback = cleaned.slice(-1200).trim();
    if (!fallback || COMMAND_ONLY_RE.test(fallback))
        return null;
    return fallback;
}
export function buildRecallHandler(getClient, cfg) {
    let lastQuery = "";
    let lastQueryTime = 0;
    let lastResult;
    let pendingQuery = "";
    ...
    return async (event) => {
        ...
        if (!query) {
            if (cfg.debug)
                console.log("[memvault] recall: skipped (no meaningful user query)");
            return;
        }
        ...
        if (query === lastQuery && now - lastQueryTime < DEDUP_TTL_MS) {
            if (cfg.debug)
                console.log("[memvault] recall: dedup hit");
            return lastResult;
        }
        if (query === pendingQuery && pendingResult) {
            if (cfg.debug)
                console.log("[memvault] recall: reusing in-flight request");
        ...
        const currentRequest = (async () => {
            ...
            const cl = getClient();
            try {
                const result = await cl.recall(currentQuery, cfg.maxRecallResults, cfg.scoreThreshold, controller.signal);
                ...
                if (parts.length === 0) {
                    if (cfg.debug)
                        console.log("[memvault] recall: no relevant memories");
                    lastResult = undefined;
                    return;
                }
                if (cfg.debug) {
                    console.log(`[memvault] recall: injecting ${result.result_count} memories`);
                }
                ...
                if (err instanceof Error && err.name === "AbortError") {
                    if (cfg.debug) {
                        console.log(`[memvault] recall: skipped (timeout after ${cfg.recallTimeoutMs}ms)`);
                    }
                }
                else if (msg.includes("circuit breaker")) {
                    if (cfg.debug)
                        console.log("[memvault] recall: skipped (server unreachable)");
                }
