 ...
 */
import { clearActivation, loadActivation, saveActivation, } from "./activation_state.js";
const LINK_TTL_MS = 24 * 3600 * 1000;
export { clearActivation, loadActivation } from "./activation_state.js";
export function isActivationExpired(activation) {
    return Date.now() - activation.created_at > LINK_TTL_MS;
}
export async function requestActivationCode(baseUrl, deviceId) {
...
export async function pollActivationStatus(baseUrl, code, deviceId) {
    try {
        const res = await fetch(`${baseUrl}/v1/activate/${encodeURIComponent(code)}/status`, {
...
export function buildActivationMessage(activation, intro = "[MemVault] 当前这台 OpenClaw 还没有连接到账户。") {
    return (`${intro}\n\n` +
