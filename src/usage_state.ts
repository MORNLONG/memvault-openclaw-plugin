...
type LocalState = {
	warn_80_fired: boolean
	warn_100_fired: boolean
}
...
function readState(): LocalState {
	...
	try {
		return JSON.parse(readFileSync(filePath, "utf-8")) as LocalState
	} catch {
		return { warn_80_fired: false, warn_100_fired: false }
	}
}
 ...
 */
export function checkAndEmitWarning(
	...
	if (maxPct >= 0.8 && !state.warn_80_fired) {
		...
		return (
			"[MemVault] Free 套餐已用约 80%，当前仍可继续使用。\n" +
