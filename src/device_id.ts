...
function normalizeLegacyId(raw: string): string | null {
	const value = raw.trim()
	if (value.startsWith("mv_device_")) return value
	if (!value.startsWith("mv_anon_")) return null
...
export function getOrCreateDeviceId(): string {
	...
	if (existsSync(devicePath)) {
		...
		if (id.startsWith("mv_device_")) return id
	}
